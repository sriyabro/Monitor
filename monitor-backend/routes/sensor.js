...
router.route('/add').post(async (req,res) => {
    ...
    try {
        ...
        let userOb = await User.findById(req.body.sensor_user).populate({path : 'cart_Items.product' , model : 'Product'})   
...
router.route('/data').post(async(req,res) => {
    ...
    let sensor = await Sensor.findOne({_id: req.body.sensor_id})
